...
def speck_scan_1(speck_apk_path, speck_final_path):
    speck_cmd = 'python3 /home/dell/zjy/08SPECK/SPECK-main/server/Scan.py -s ' + speck_apk_path
...
def speck_scan(speck_apk_path, speck_final_path):
    try:
        speak_path = '/home/dell/zjy/tool_overlap_0311/speak_err.txt'
        speak_file = open(speak_path,'a')
        speck_cmd = 'python3 /home/dell/zjy/08SPECK/SPECK-main/server/Scan.py -s ' + speck_apk_path
        ...
        if not (os.path.exists(speck_report_path)):
            os.mkdir(speck_report_path)
        # log_1 = open('speak_log_output.txt','w+')
        # log_1.write('speak log file \n')
        # log_1.flush()
        # log_2 = open('speak_log_err.txt','w+')
        # log_2.write('speak log file \n')
        # log_2.flush()
        # timeout = 720
        # p = subprocess.Popen(speck_cmd,
        #                     stdout=log_1,
        ...
        #         logging.info('scan successfully in'+str(speck_apk_name))
        #         break
        #     seconds_passed = time.time() - t_beginning
        #     if timeout and seconds_passed > timeout:  
        #         p.terminate()
        #         msg = 'This app '+ speck_apk_name + ' is timeout [speak]!'
        #         logging.warning(msg)
        #         break
        #     time.sleep(1)
        # p.wait()
        ...
        # err = log_2.read()
        # if err:
        #     logging.critical('stop in wrong'+speck_apk_path)
        # output = log_1.read()
        # speak_file.write(speck_apk_path)
...
def speck_batch(speck_apks_path, speck_final_path):
    ...
    for i in range(len(speck_apks_list)):
        try:
            ...
            time_path = '/home/dell/zjy/tool_overlap_0311/Time_Record_0721/Speck_time_record.txt'
            print(different_time)
            time_file = open(time_path,'a')
            time_file.write(speck_apks_list[i]+' : ')
