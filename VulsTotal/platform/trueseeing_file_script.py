...
def trueseeing_run(folder_path,report_folder):
    ...
    trueseeing_folder = os.path.join(os.path.dirname(current_dir),'trueseeing-master')
    # os.chdir(trueseeing_folder)
    apk_list = os.listdir(folder_path)
    logger.info('Trueseeing Begin Scanning')
    des_path = ''
    ...
    for i in range(len(apk_list)):
        if(apk_list[i].endswith('.apk')):
            try:
                ...
                logger.info('[TrueSeeing] Begin to scanning: '+ str(apk_list[i])+ ' '+ str(i+1)+'/'+str(len(apk_list)))
                print(trueseeing_cmd)
