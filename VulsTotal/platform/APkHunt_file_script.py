...
def APKHunt_run(folder_path,report_folder):
    apkhunt_filer = 'VulsTotal/APKHunt/apkhunt.go'
    apk_list = os.listdir(folder_path)
    logger.info('APKHunt Begin Scanning')
    ...
    for i in range(len(apk_list)):
        if(apk_list[i].endswith('.apk')):
            ...
            logger.info('[APKhunt] Begin to scanning: ' + str(apk_list[i])+ ' '+ str(i+1)+'/'+str(len(apk_list)))
