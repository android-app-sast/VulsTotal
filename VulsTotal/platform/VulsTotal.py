...
def copy_to_AUSERA(ausera_main_apks,apks_folder):
    ...
    # logger.info(' [AUSERA] Copy to the AUSERA file process done')
    ...
    for i in range(len(apk_name)):
        try:
            ...
            if index_1 == 2 or index_1 ==0:
                ...
                qark_exec.sendeof()
                logger.critical('\033[31m [QARK] Timeout for decomplie.')
            endTime = time.time()
...
def super_show(apks_folder, reports_folder):
    try:
        ...
        logger.critical('\033[1;31m [SUPER] Exists the error in SUPER.')
        # traceback.print_exc()
...
def scan(reports_folder, apks_folder):
    ...
    logger.info(' All apps are scanned finally! ')
...
def run(apk_folder, stra_option, confiden_option):
    ...
    logger.info(' [VulsTotal Scanning ] Analyzing... Please wait for a moment.')
...
if __name__ == '__main__':
    ...
    run(apk_folder, stra_option, confiden_option)
